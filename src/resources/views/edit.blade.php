<x-admin.layout.app>
    ...
    $breadcrumbs = [
        ...
        ['name' => $element->title],
        ['name' => "Gallery Content", "link" => route('LaravelBlock2Gallery.index', $element->id)],
        ['name' => "Edit"]
    ];
    @endphp
    ...
    <form
        class="relative mt-7"
        method="POST"
        action="{{ route("LaravelBlock2Gallery.update", ["element_id" => $element->id, "id" => $collection->id]) }}"
    >
        @csrf
        @method("patch")
        ...
        <x-admin.atoms.row>
            ...
            <input
                ...
                data-uic-path="{{ $collection->getMedia('laravel_block2_gallery')->first()->getUrl() }}"
            />
